...
var ZERO_DATE time.Time = time.Date(1, 1, 1, 0, 0, 0, 0, time.UTC)

func TestNewDeck(t *testing.T) {
	d := deck.NewDeck(ZERO_DATE)
...
func TestMeta(t *testing.T) {
	a := core.NewMeta("2020-01-01T00:00:00Z", "0", "sm2", []string{"2.5"})
...
func TestInsertCard(t *testing.T) {
	...
	if c, err := NewCards("", f5); err != nil {
		...
		if e := d.InsertCard(c[0], 5); e != nil {
			panic("Should have inserted.")
		}
		if d.TopCard().HashStr() != c[0].HashStr() {
			panic("Card not inserted.")
		}
	}
	...
	if c, err := NewCards("", f2); err != nil {
		...
		if e := d.InsertCard(c[0], -100); e != nil {
			panic("Should have inserted.")
		}
		if d.TopCard().HashStr() != c[0].HashStr() {
			panic("Card not inserted.")
